...
namespace SaccFlightAndVehicles
{
    ...
    public class SaccAirVehicle : UdonSharpBehaviour
    {
        ...
        void Start()
        {
            var all = GetComponents<SaccAirVehicle>();
            Debug.Log($"SAV COMPONENT COUNT | count={all.Length}");
            ...
            set
            {
                ...
                if (value)
                {
                    if (!_EngineOn)
                    {
                        ...
                        Debug.Log("SAV STEP 2 after SetBool");
                        WakeUp();

                        Debug.Log("SAV STEP 3 after WakeUp");
                        ...
                        Debug.Log("SAV STEP 4 after thrust init");
...
# else
        ...
        public void SFEXT_L_EntityStart()
        {
            /*Debug.Log(
                ...
                $"rbNull={VehicleRigidbody == null} | " +
                $"entityNull={EntityControl == null}"
            );
            */
            Initialized = true;
            ...
            /*
            Debug.Log(
                $"SAV INIT PHASE 2 RB SET | " +
                $"animNull={VehicleAnimator == null} | " +
                $"rbNull={VehicleRigidbody == null} | " +
                $"rbObj={(VehicleRigidbody != null ? VehicleRigidbody.gameObject.name : "null")}"
            );
            */
            ...
            /*
            Debug.Log(
                $"SAV INIT PHASE 3 ANIM SET | " +
                $"animNull={VehicleAnimator == null} | " +
                $"rbNull={VehicleRigidbody == null} | " +
                $"animObj={(VehicleAnimator != null ? VehicleAnimator.gameObject.name : "null")}"
            );
            */
            FullHealth = Health;
            FullFuel = Fuel;
            ...
            AtmoshpereFadeDistance = (AtmosphereThinningEnd + SeaLevel) - (AtmosphereThinningStart + SeaLevel);
            AtmosphereHeightThing = (AtmosphereThinningStart + SeaLevel) / (AtmoshpereFadeDistance);
            ...
            Debug.Log(
                $"SAV INIT PHASE 4 EXIT | " +
                $"selfCompId={GetInstanceID()} | " +
                $"selfGoId={gameObject.GetInstanceID()} | " +
                $"selfObj={gameObject.name} | " +
                $"selfPath={GetPath(transform)} | " +
                $"animNull={VehicleAnimator == null} | " +
                $"rbNull={VehicleRigidbody == null} | " +
                $"animObj={(VehicleAnimator != null ? VehicleAnimator.gameObject.name : "null")} | " +
                $"rbObj={(VehicleRigidbody != null ? VehicleRigidbody.gameObject.name : "null")}"
